    ...
    def __init__(self):
        ...
        self.agent = Agent(
            ...
            system_prompt=(
                ...
                "- kb.get_directly_left(name) -> str | None (person directly left, or None)\n"
                "- kb.get_directly_right(name) -> str | None (person directly right, or None)\n"
                "- kb.get_directly_above(name) -> str | None (person directly above, or None)\n"
                "- kb.get_directly_below(name) -> str | None (person directly below, or None)\n\n"
                "--- Counters ---\n"
                "- kb.count_criminals(names: List[str]) -> Z3 ArithRef\n"
                "- kb.count_innocents(names: List[str]) -> Z3 ArithRef\n"
                "- kb.count_profession(profession: str | List[str]) -> int\n"
                "  Returns count of people with profession (if str) or length of name list (if list)\n\n"
                ...
                "    And(\n"
                "        kb.count_innocents(kb.get_row(1)) == 2,\n"
                "        kb.row_connected(1, status_is_criminal=False)\n"
                "    )\n"
                "15. DO NOT filter lists using Z3 conditions in Python 'if' statements. Z3 variables are not booleans.\n"
                "    WRONG: len([p for p in people if kb.is_innocent(p)]) == 1\n"
                "    RIGHT: Sum([If(kb.is_innocent(p), 1, 0) for p in people]) == 1\n"
                "    Example: 'Exactly 1 pilot has an innocent below them' ->\n"
                "    Sum([\n"
                "        If(Or([And(kb.is_directly_below(n, p), kb.is_innocent(n))\n"
                "               for n in kb.get_below(p)]), 1, 0)\n"
                "        for p in kb.get_profession('pilot')\n"
                "    ]) == 1\n\n"
                "--- Examples ---\n"
                ...
                "Clue: 'Exactly 1 doctor has an innocent directly to the right of them'\n"
                "Code: Sum([\n"
                "    If(And(kb.get_directly_right(p) != None,\n"
                "           kb.is_innocent(kb.get_directly_right(p))), 1, 0)\n"
                "    for p in kb.get_profession('doctor')\n"
                "]) == 1\n\n"
                "Clue: '2 of the 3 painters have an innocent directly to the right of them'\n"
                "Code: And(\n"
                "    kb.count_profession('painter') == 3,\n"
                "    Sum([\n"
                "        If(And(kb.get_directly_right(p) != None,\n"
                "               kb.is_innocent(kb.get_directly_right(p))), 1, 0)\n"
                "        for p in kb.get_profession('painter')\n"
                "    ]) == 2\n"
                ")\n\n"
